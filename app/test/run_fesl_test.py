...
from app.models.fesl_types import FeslType
...
async def setup_database():
    ...
    try:
        ...
        user_data = UserCreate(username=TEST_USERNAME, password=TEST_PASSWORD, email=TEST_EMAIL)
    ...
    try:
        ...
        if payload.get("TXN") == "NuLogin":
            ...
            if user_id == profile_id:
                print("       ✓ userId == profileId (correct after NuLogin)")
            else:
                print("       ✗ userId != profileId (unexpected)")
        ...
        if payload.get("TXN") == "NuLoginPersona":
            ...
            if str(expected_user_id) == new_user_id:
                print("       ✓ userId stayed same after persona selection")

            # Note: userId and profileId might be equal if user.id == persona.id
            if new_user_id != new_profile_id:
                print("       ✓ userId != profileId (profileId is now persona ID)")
            else:
                print("       ! userId == profileId (IDs happen to match numerically)")
            ...
            try:
                ...
                if len(parts) == 3:
                    ...
                    if ticket_user_id == str(expected_user_id):
                        print("       ✓ Ticket userId matches expected")
                    if ticket_persona_id == str(expected_persona_id):
                        print("       ✓ Ticket personaId matches expected")
                else:
